...
import com.baize.mybatis.reverse.util.FileUtil;
import com.baize.mybatis.reverse.util.VelocityUtil;
...
public class Maker {
    ...
    public Maker() {
        try {
            ...
            VelocityUtil.initVelocityEngine(userDir + File.separator + "mysql-reverse/src/main/java/com/wisdom/app/mysql/reverse/resource");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//				xml
                makeFile(data, "xml.vm", outputRootProvider + File.separator + ".." + File.separator + "resources" + File.separator + "mybatis", CodeUtil.convertClassNameToPath((String) data.get(
                        "daoName"), "xml"));
            }
        }
    }

    protected void makeJava(Map<String, Object> data, String basePackage, String group, String tableName, String tablePrefix, String tablePostfix, String common, String variableSuffix,
                            String outputRoot,
                            String template) {
        String classPackage = CodeUtil.getPackage(basePackage, common, group);
        String variable = CodeUtil.getVariable(tableName.substring(tablePrefix.length(), tableName.length() - tablePostfix.length())) + variableSuffix;
